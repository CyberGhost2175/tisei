...
export default function MapPage() {
  ...
  const load = useCallback(
    async (silent = false) => {
      ...
      try {
        const executorId = user?.role === "executor" ? user.id : undefined;
      ...
      <div className="flex flex-col md:flex-row min-h-[calc(100vh-4rem)]">
        <aside className="w-full md:w-[340px] lg:w-[380px] shrink-0 bg-surface border-r border-outline-variant flex flex-col z-20 max-h-[42vh] md:max-h-none md:h-[calc(100vh-4rem)]">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            ...
            {user?.role === "executor" && points.length === 0 && !loading && (
            ...
            {displayPoints.map((p, idx) => (
              ...
              >
                <div
                  className={
                    p.isPartner
                      ? "w-8 h-8 rounded-full bg-[#e65100] text-white flex items-center justify-center font-bold text-sm shrink-0"
